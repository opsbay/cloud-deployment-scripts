...
if [[ ! "${rs_status_rc}" -eq "0" ]]; then
  if [[ $(mongo --quiet --eval "JSON.stringify(rs.status())" | jq -r .code) == "94" ]]; then
    ...
    if [[ $(mongo --quiet --eval "JSON.stringify(db.isMaster());" | jq -r .ismaster) == "true" ]]; then
      mongo \
        --quiet \
        --eval "db.createUser({ user: \"${MONGODB_ADMIN_USERNAME}\", pwd: \"${MONGODB_ADMIN_PASSWORD}\", roles: [ \"userAdminAnyDatabase\",\"dbAdminAnyDatabase\",\"clusterAdmin\",\"readWriteAnyDatabase\"]}) admin"
      mongo \
        --quiet \
        -u "${MONGODB_ADMIN_USERNAME}" \
        -p "${MONGODB_ADMIN_PASSWORD}" \
      ...
      mongo \
        ...
        -u "${MONGODB_ADMIN_USERNAME}" \
        -p "${MONGODB_ADMIN_PASSWORD}" \
        --eval "db.createUser({user: \"${MONGODB_USERNAME}\", pwd: \"${MONGODB_PASSWORD}\",roles: [{ role: \"readWrite\", db: \"${MONGODB_DBNAME}\" }]}) admin"
    fi
  fi
fi
