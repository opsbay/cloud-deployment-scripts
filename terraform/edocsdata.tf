module "edocsdata" {
  ...
  lb_security_groups = [
    "${module.vpc.default_security_group_id}",
    "${aws_security_group.edocsdata-service.id}",
  ...
  instance_types                = ["${var.edocsdata_instance_types}"]
...
resource "aws_security_group" "edocsdata-service" {
  name        = "tf-edocsdata-service-sg"
  description = "Allows access to eDocs API Data service and healthcheck"
  vpc_id      = "${module.vpc.vpc_id}"
