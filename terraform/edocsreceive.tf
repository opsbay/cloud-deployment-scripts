module "edocsreceive" {
  ...
  lb_security_groups = [
    "${module.vpc.default_security_group_id}",
    "${aws_security_group.edocsreceive-service.id}",
  ...
  instance_types                = ["${var.edocsreceive_instance_types}"]
  autoscaling_capacity_defaults = ["${var.edocs_autoscaling_capacity_defaults}"]
...
resource "aws_security_group" "edocsreceive-service" {
  name        = "tf-edocsreceive-service-asg"
  description = "Allows access to eDocsAPI Receive service and healthcheck"
  vpc_id      = "${module.vpc.vpc_id}"
