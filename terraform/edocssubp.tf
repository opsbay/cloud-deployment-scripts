module "edocssubp" {
  ...
  lb_security_groups = [
    "${module.vpc.default_security_group_id}",
    "${aws_security_group.edocssubp-service.id}",
  ...
  instance_types                = ["${var.edocssubp_instance_types}"]
...
resource "aws_security_group" "edocssubp-service" {
  name        = "tf-edocssubp-service-sg"
