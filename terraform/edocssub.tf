module "edocssub" {
  ...
  lb_security_groups = [
    "${module.vpc.default_security_group_id}",
    "${aws_security_group.edocssub-service.id}",
  ...
  instance_types                = ["${var.edocssub_instance_types}"]
...
resource "aws_security_group" "edocssub-service" {
  name        = "tf-edocssub-service-sg"
