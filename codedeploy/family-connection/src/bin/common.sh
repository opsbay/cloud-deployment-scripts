...
export DOCUMENT_ROOT="$APP_DIR/wk12"
...
get_server_name_json() {
    local server_name
    local succeed_server_name

    # FIXME: This has to be fixed before the actual cutover because we are hardcoding some values
    # in for now.
    case "$DEPLOYMENT_GROUP_NAME" in
        staging|qa)
            server_name="tf-connection-$PHP_VERSION-$DEPLOYMENT_GROUP_NAME.mango.naviance.com"
            succeed_server_name="tf-succeed-$PHP_VERSION-$DEPLOYMENT_GROUP_NAME.mango.naviance.com"
        ;;

        preprod)
            server_name="tf-connection-$PHP_VERSION-$DEPLOYMENT_GROUP_NAME.papaya.naviance.com"
            succeed_server_name="tf-succeed-$PHP_VERSION-$DEPLOYMENT_GROUP_NAME.papaya.naviance.com"
        ;;

        production)
            server_name="connection.naviance.com"
            succeed_server_name="succeed.naviance.com"
        ;;
    ...
    cat <<EMITTED_JSON
{
    ...
    "succeedServerName": "$succeed_server_name"
}
EMITTED_JSON
}
