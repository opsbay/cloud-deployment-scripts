...
APP_DIR="${APP_DIR}"
INPUT_J2="$APP_DIR/application-standard.j2"
INPUT_DATA="${APP_DIR}/input_data.json"
AURORA_CONFIG=$(get_config_by_env aurora)
CLUSTER_ENDPOINT="$APP_DIR/aurora-cluster-endpoint.json"

aws s3 cp "$AURORA_CONFIG" "$CLUSTER_ENDPOINT"
aws s3 cp "$bucket/${environment}/db/naviance.json" "$APP_DIR"
aws s3 cp "$bucket/${environment}/${APP_NAME}/application-standard.j2" "$APP_DIR"

# Combine JSON files for jinja2
# Thanks Stack Overflow https://stackoverflow.com/a/36218044/424301
#shellcheck disable=SC2016
cd "$APP_DIR" && cat "$CLUSTER_ENDPOINT" "naviance.json"   \
    | jq --slurp 'reduce .[] as $item ({}; . + $item)'                     \
    > "$INPUT_DATA"

jinja2 "$INPUT_J2" "$INPUT_DATA" --format=json > "$CONFIG"
chown "root:$APP_USER" "$CONFIG"
chmod 640 "$CONFIG"
