...
mkdir -p "$app_config/app" "$app_config/php/56"
...
aws s3 cp "s3://$bucket/${environment}/$APP_NAME/app/crmConfig.php.j2"        "$app_config/app"
...
aws s3 cp "s3://$bucket/${environment}/$APP_NAME/php/memcache.ini"         "$app_config/php"
aws s3 cp "s3://$bucket/${environment}/$APP_NAME/php/php.ini.j2"           "$app_config/php"

mkdir -p "${APP_DIR}/config/"
mkdir -p "${APP_DIR}/succeed-legacy/config/"
mkdir -p "${APP_DIR}/succeed-legacy/core/config/"
cp "$app_config/app/"{mapquestConfig,salesforceConfig}.php "${APP_DIR}/succeed-legacy/config/"
mv "$app_config/app/"{mapquestConfig,salesforceConfig}.php "${APP_DIR}/config/"
...
jinja2 \
    ...
    "$input_data" \
    --format=json \
    > "${APP_DIR}/succeed-legacy/core/config/config.xml"

jinja2 \
    "$app_config/app/crmConfig.php.j2" \
    "$input_data" \
    --format=json \
    > "${APP_DIR}/config/crmConfig.php"
cp "${APP_DIR}/config/crmConfig.php" "${APP_DIR}/succeed-legacy/config/"
...
cp "${APP_DIR}/config/dbConfig.php" "${APP_DIR}/succeed-legacy/config/"
...
cp "${APP_DIR}/config/wk12Config.php" "${APP_DIR}/succeed-legacy/config/"
