...
aws s3 cp "s3://$bucket/${environment}/$APP_NAME/db.php.j2"       "${app_config}/"
aws s3 cp "s3://$bucket/${environment}/$APP_NAME/users.php"       "${app_config}/"
...
mv "$app_config/users.php" "${APP_DIR}/am/config/"
jinja2 \
    "$app_config/db.php.j2" \
    "$input_data" \
    --format=json \
    > "${APP_DIR}/am/config/db.php"
