...
# Configure New Relic
unzip -o "$DIR/$newrelic_zip" -d "$APP_DIR"
...
chown -R "root:$JAVA_CRON_USER" "$newrelic_dir"
chown "$JAVA_CRON_USER:$JAVA_CRON_GROUP" "$newrelic_dir/logs"
setfacl -m "u:newrelic:rwx" "$newrelic_dir/logs"
