...
# Important: This script must be copied to one of the paths referenced by mongo-cluster_init.tpl.sh.
...
NODE_NAME=$(aws ec2 describe-instances --instance-ids "${EC2_INSTANCE_ID}" --region us-east-1 | jq -r '.Reservations[].Instances[].Tags[] | select(.Key=="SimpleName") | .Value')
...
S3_MONGO_CONFIG_PATH="${S3_BUCKET}/${ENVIRONMENT}/${NODE_NAME}"
...
function update-config() {
  try_with_backoff aws s3 cp "${S3_MONGO_CONFIG_PATH}/mongod.keyfile" /etc/mongod.keyfile
  try_with_backoff aws s3 cp "${S3_MONGO_CONFIG_PATH}/mongod.conf" /etc/mongod.conf
...
if [[ ! $(semodule -l | grep -i "mongod.1.0") ]]; then
  try_with_backoff aws s3 cp "${S3_MONGO_CONFIG_PATH}/mongod.te" .
...
try_with_backoff aws s3 cp "${S3_MONGO_CONFIG_PATH}/credentials" .
...
if [[ ! "${rs_status_rc}" -eq "0" ]]; then
  if [[ $(mongo --quiet --eval "JSON.stringify(rs.status())" | jq -r .code) == "94" ]]; then
    ...
    RS_INIT_CONFIG="rs.initiate({_id : \"${ENVIRONMENT}\", members: [{ _id : 0, host : \"tf-${NODE_NAME}-${ENVIRONMENT}-00.local.naviance.com:27017\" }, { _id : 1, host : \"tf-${NODE_NAME}-${ENVIRONMENT}-01.local.naviance.com:27017\" }, { _id : 2, host : \"tf-${NODE_NAME}-${ENVIRONMENT}-02.local.naviance.com:27017\" }]});"
    ...
    if [[ $(mongo --quiet --eval "JSON.stringify(db.isMaster());" | jq -r .ismaster) == "true" ]]; then
      mongo admin \
        --quiet \
        --eval "db.createUser({ user: \"${MONGODB_ADMIN_USERNAME}\", pwd: \"${MONGODB_ADMIN_PASSWORD}\", roles: [ \"root\"]})"
    fi
  fi
fi
